...
from api.models import Inventory, Invoice, Owner, Part, Report, Task, TaskTemplate, Vehicle
...
class ReportViewSetTest(APITestCase):
    ...
    def setUp(self):
        ...
        v1 = Vehicle.objects.create(
            owner=self.owner, brand="Audi", model="A3", year=2010, license_plate="XYZ1"
        )
        v2 = Vehicle.objects.create(
            owner=self.owner, brand="BMW", model="X5", year=2005, license_plate="XYZ2"
        )
        ...
        self.task_template = TaskTemplate.objects.create(
            name="Oil Change", description="Change engine oil", price=50
        )
        self.task = Task.objects.create(report=self.report1, task_template=self.task_template)

        self.inventory = Inventory.objects.create(
            name="Oil Filter", quantity_in_stock=20, unit_price=15
        )

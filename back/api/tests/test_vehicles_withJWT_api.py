...
from api.models import Owner, Vehicle
...
class VehicleTests(APITestCase):
    ...
    def setUp(self):
        ...
        login_response = self.client.post(
            self.login_url, {"email": "john@example.com", "password": "strongpassword123"}
        )
    ...
    def test_list_vehicles(self):
        ...
        Vehicle.objects.create(
            brand="Honda", model="Civic", year=2019, license_plate="ABC 789", owner=self.owner
        )
    ...
    def test_update_vehicle(self):
        ...
        vehicle = Vehicle.objects.create(
            brand="Ford", model="Focus", year=2018, license_plate="DEF 456", owner=self.owner
        )
    ...
    def test_delete_vehicle(self):
        ...
        # Create a vehicle in the database
        vehicle = Vehicle.objects.create(
            brand="Nissan", model="Altima", year=2020, license_plate="GHI 789", owner=self.owner
        )
        url = reverse(
            "vehicle-detail", args=[vehicle.id]
        )  # Assuming you have a route for vehicle detail

...
from api.models import Owner, User, Vehicle
...
class VehiclePermissionsTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.owner_a = Owner.objects.create(
            first_name="User", last_name="A", email="a@example.com", phone="1234567890"
        )
        ...
        self.owner_b = Owner.objects.create(
            first_name="User", last_name="B", email="b@example.com", phone="9876543210"
        )

        # Vehicle belonging to User A's Owner
        self.vehicle = Vehicle.objects.create(
            brand="BMW", model="320i", year=2022, license_plate="A123XYZ", owner=self.owner_a
        )

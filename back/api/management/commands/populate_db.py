...
from api.models import Inventory, Owner, Part, Report, Task, TaskTemplate, User, Vehicle
...
with open(brands_models_path) as file:
    brands_models = json.load(file)
    brands_list = list(brands_models.keys())

with open(inventory_data_path) as file:
    inventory_data = json.load(file)

with open(tasks_data_path) as file:
...
def populate_inventory():
    for item in inventory_data:
        ...
        inventory, created = Inventory.objects.get_or_create(
            reference_code=item["reference_code"], defaults=item
        )
        if created:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--vehicles", action="store_true", help="Populate vehicles (requires owners)"
        )
        parser.add_argument(
            "--reports", action="store_true", help="Populate reports (requires users and vehicles)"
        )

export const filterItems = (
  ...
  if (filters.type === 'report') {
    ...
    return (
      ...
      (filters.created_at === '' || item.formatted_created_at.includes(filters.created_at)) &&
  ...
  if (filters.type === 'owner') {
    return (
      (filters.name === '' || item.full_name.toLowerCase().includes(filters.name)) &&
  ...
  if (filters.type === 'vehicle') {
    return (
      ...
      (filters.license_plate === '' ||
        item.license_plate.toLowerCase().includes(filters.license_plate.toLowerCase())) &&
      (filters.vehicle_owner === '' ||
        (item.owner && item.owner.toString().includes(filters.vehicle_owner.toString())))
    )
  }
  ...
  if (filters.type === 'inventory') {
    return (
      ...
      (filters.reference_code === '' ||
        item.reference_code.toLowerCase().includes(filters.reference_code.toLowerCase())) &&
      (filters.category === '' || item.category.includes(filters.category)) &&
      (filters.updated_at === '' || item.formatted_updated_at.includes(filters.updated_at))
    )
  }

...
export function isValidYear(year) {
  ...
  if (!yearRegex.test(year)) return `Year must be between 1900 and ${currentYear}.`
...
export const isValidOrTakenLicensePlate = (licensePlate, existingLicensePlates) => {
  if (!licensePlate.trim()) return 'This field is required.'
  if (existingLicensePlates.some((plate) => plate.toLowerCase() === licensePlate.toLowerCase())) {
    return 'License plate is already taken.'
  }

  const licensePlateRegex = /^[A-Z0-9-\s]{2,10}$/
  if (!licensePlateRegex.test(licensePlate)) return 'Invalid license plate format.'
